...
def make_auth_tab():
    error=pd.read_csv('~/Logging/LogFiles/auth/error.csv')
    system=pd.read_csv('~/Logging/LogFiles/auth/system.csv')
    log=pd.read_csv('~/Logging/LogFiles/auth/auth_log.csv')
    ...
    labels = ['Error Messages','System Authorizations','All']
    ...
    # Make a tab with the layout 
    tab = Panel(child=layout, title = 'Authorization Details')
    return tab
    # show(layout)

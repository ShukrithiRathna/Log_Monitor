...
def make_kernel_tab():
    error=pd.read_csv('~/Logging/LogFiles/kern/error.csv')
    system=pd.read_csv('~/Logging/LogFiles/kern/system.csv')
    log=pd.read_csv('~/Logging/LogFiles/kern/kern_log.csv')
    ...
    def make_plot_table(src_table):
        table_columns = [TableColumn(field='Time', title='Timestamp'),TableColumn(field='User', title='User'),TableColumn(field='Message', title='Message')]
    ...
    labels = ['Error Messages','System Messages','All']
    ...
    tab = Panel(child=layout, title = 'Kernel Log')
    return tab
    # show(layout)
